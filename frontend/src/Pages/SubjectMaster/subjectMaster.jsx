...
const SubjectMaster = () => {
  ...
  const [searchQueryClassName, setSearchQueryClassName] = useState({
    ...useLocation().state,
  });
  ...
  useEffect(() => {
    if (searchQueryClassName?.class_name) {
      setSearchQuery(searchQueryClassName?.class_name);
    }
  }, []);
  const {
  ...
  const deleteMultipleSubjects = async () => {
    try {
      ...
      await SubjectService.deleteSubject(itemsToDelete);
  ...
  const ChangeStatus = async (id, status) => {
    try {
      await SubjectService.updateSubjectStatus(id, status);
  ...
  return (
    <div>
      <div>
        <div className="content" data-number="1">
          <div className="mb-3 d-flex justify-content-between">
            <h1 className="black fs-18 fw-600 mb-0">SUBJECT MASTER</h1>
            <div className="d-flex">
              <h4 className="fs-12 fw-500 black mb-0 me-1">Dashboard &gt;</h4>
              <h4 className="fs-12 fw-500 primary mb-0">Subject Master</h4>
            </div>
          </div>
          ...
          <div className="table_container" style={{ display: "block" }}>
            <div className="table_title">
              <div className="table_headings">
                <h1 className="black fs-18 fw-600 mb-0">Subjects List</h1>
              ...
              >
                <thead>
                  <tr className="table_heading">
                    ...
                    <th scope="col" style={{ width: "30%" }} className="pe-auto">
                      View
                    </th>
                ...
                <tbody className="bg_light">
                  {data?.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td key={`${item.id} ${item.subject_name}`}>
                        {item.subject_name}
                      </td>
                      <td key={`${item.id} ${item.class_name}`}>
                        {item.class_name}
                      </td>
                      <td className="link">
                        <a
                          className="link-opacity-100 pe-auto"
                          onClick={() =>
                            navigate(RoutesPath.viewStudyMaterial, {
                              state: item,
                            })
                          }
                        >
      ...
      <ConfirmationModal
        ...
        text={
          isMultipleDelete
            ? "Are you sure want to delete selected subjects?"
